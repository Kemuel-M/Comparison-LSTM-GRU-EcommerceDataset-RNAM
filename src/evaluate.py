...
import numpy as np
import pandas as pd
from utils import calculate_metrics, plot_predictions, plot_training_history, count_parameters
import config
...
def evaluate_model(model, predictions, targets, history, training_time, model_name):
    ...
    try:
        save_path = config.RESULTS_DIR / f"{model_name}_predictions.png"
        plot_predictions(target_flat, pred_flat, model_name, save_path)
        print(f"Gráfico de previsões salvo em: {save_path}")

        # Plotar histórico
        save_path = config.RESULTS_DIR / f"{model_name}_history.png"
